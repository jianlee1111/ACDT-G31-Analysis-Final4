...
data_path = '/content/gdrive/MyDrive/ACDT final data set22.xlsx - 데이터.csv'
...
if not df_cleaned.empty:
    ...
    df = pd.read_csv(data_path) # Reload df to ensure it's available
...
"""## Streamlit 앱 실행 코드 생성

### Subtask:
Streamlit 앱을 실행하는 코드를 생성합니다.
"""
...
**Reasoning**:
Add the provided markdown text explanations to the Streamlit app using `st.markdown()`, placing each block in the appropriate section relative to the analysis results it describes.
"""
...
**Reasoning**:
Add a section title for regression analysis, integrate the code to build the four regression models using `statsmodels.formula.api.ols`, display the summary of each model using `st.text`, add a brief interpretation below each summary, and include error handling. Ensure the `df_regression_renamed` DataFrame is available for the analysis.
"""
...
**Reasoning**:
Add a section title for descriptive statistics, select numeric columns, calculate descriptive statistics, and display the results in the Streamlit app. Then, add a section title for variable distribution visualization and iterate through the numeric columns of the cleaned dataframe to create and display histograms and box plots for each variable using Streamlit's plotting function.
"""
...
### Subtask:
기존 데이터 로딩 및 전처리 코드를 Streamlit 앱에 통합합니다. 데이터가 한 번 로드되고 전처리되도록 함수로 캡슐화하는 것을 고려합니다.

**Reasoning**:
Encapsulate the data loading and preprocessing logic into a function and apply Streamlit's caching decorator to it. Then, call this function and display the head and info of the resulting DataFrame in the Streamlit app.
"""
...
"""**Reasoning**:
The previous step identified the columns for some variables but missed others like R&D Researcher, High-tech Export Share, GDP, Total Population, and Labor Force. This step attempted to refine the column identification process based on the first row values, considering variations in naming, and then finalize the dictionary of identified columns.
...
"""**Reasoning**:
The previous step successfully identified most of the required columns. We refined the search for the missing variables and select the most relevant 'Real GDP' columns, potentially focusing on the most recent years if not specified, and then confirmed all necessary columns are captured.
...
"""**Reasoning**:
The previous attempt did not find columns for 'Non-Resident Patent Applications', 'Total Population', 'Labor Force Size', and 'Business R&D Personnel (FTE) per 1,000 Employed'. We examined the first row values, looking for slightly different phrasing or combined columns, and refine the search for these specific variables. We than confirmed if all necessary columns are now identified. We also specifically look for a column header containing 'Non-Resident Patent Applications' or inferred it if it's in the same column as 'Resident Patent Applications'. We also tried to find 'Business R&D Personnel (FTE) per 1,000 Employed' specifically.
...
"""**Reasoning**:
The previous steps successfully identified most of the required columns, but 'Total Population', 'Labor Force Size', and 'Business R&D Personnel (FTE) per 1,000 Employed' are still missing. We performed a final, more flexible search for these columns by looking for keywords within the entire first row string for each column, and then confirmed the final list of identified columns. The subtask is to locate the columns, so confirming the final set completes the subtask.
...
Clean the data in the identified columns by removing non-numeric characters and converting the data to a numeric format. Handle any missing values appropriately.

**Reasoning**:
Clean the data in the identified columns by removing non-numeric characters and converting the data to a numeric format. Handle any missing values by dropping rows with NaNs in the selected variable columns.
"""
...
**Reasoning**:
Select numeric columns from the cleaned DataFrame and calculate descriptive statistics.
"""
...
"""**Reasoning**:
Create histograms and box plots for each numeric variable in the cleaned dataframe to visualize their distributions and identify potential outliers.
...
We cleaned the data by removing non-numeric characters and converting to numeric, handling missing values
"""
...
# Task
Convert the provided Python notebook code and Markdown text into a Streamlit application that displays the analysis results and accompanying text.

## Streamlit 애플리케이션 구조 정의

### Subtask:
Streamlit 앱의 기본 구조를 설정합니다. 필요한 라이브러리를 가져오고 페이지 레이아웃을 정의합니다.

**Reasoning**:
Set up the basic structure of the Streamlit application by importing necessary libraries, configuring the page layout, adding a title, and adding a sidebar with a description.
"""
...
**Reasoning**:
Now that streamlit is installed, set up the basic structure of the Streamlit application by importing necessary libraries, configuring the page layout, adding a title, and adding a sidebar with a description.
"""
...
"""**Reasoning**:
The error "ValueError: Cannot describe a DataFrame without columns" indicates that the `df_numeric` DataFrame is empty, meaning no numeric columns were selected from `df_cleaned`. This could be because `df_cleaned` is empty or does not contain any numeric columns after the cleaning process. Before attempting to calculate descriptive statistics or plot, check if `df_cleaned` is empty and if it contains any numeric columns. If it's not empty but has no numeric columns, print a message indicating this. If it is empty, print a message indicating that data loading or cleaning failed.
